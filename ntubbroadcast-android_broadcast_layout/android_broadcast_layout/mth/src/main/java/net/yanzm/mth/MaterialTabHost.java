...
//        int indicatorColor = a.getColor(R.styleable.MaterialTabHost_colorTabIndicator, colorControlActivated);
        int indicatorColor = Color.rgb(175, 85, 0);
        a.recycle();
...
//        int tabHeight = res.getDimensionPixelSize(R.dimen.mth_tab_height);
     ...
     * add new tab with title text
     *
     * @param image title text
     */
    public void addTab(int image) {
        int layoutId = getLayoutId(type);
        ImageView iv = (ImageView) inflater.inflate(layoutId, tabWidget, false);
        iv.setImageResource(image);

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            iv.setBackgroundResource(R.drawable.mth_tab_widget_background_ripple);
